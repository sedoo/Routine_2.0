    ...
    def etatActuel(self):
        ...
        for evenement in xmlfic.getElementsByTagName('evenement'):
            ...
            if evenement.getElementsByTagName('fin'):
                ...
                for key,arg in list(etatActuel.items()):
                    p = {'OK' : 0, '???' : 1, 'Pb' : 2, 'HS' : 3, '' : 4}
                    if evtActuel[key] in p:
...
#            else:
    ...
    def listeInt(self):
        '''Methode affichant l'historique des interventions'''
        ''' interventions [
                {
                    'intervenant': '...'
                    'date': '...'
        ...
        for interv in xmlfic.getElementsByTagName('intervention'):
            ...
            intervention['evt'] = []
            intervention['nbEvt'] = interv.getElementsByTagName('evt').length
            for evt in interv.getElementsByTagName('evt'):

                idEvt = evt.attributes['idEvt'].value
                for evenement in xmlfic.getElementsByTagName('evenement'):
                    if evenement.attributes['idEvt'].value == idEvt:
                        description = evenement.getElementsByTagName('description')[0].firstChild.nodeValue
            ...
            #lstInterventions.append(intervention)
            lstInterventions.insert(0, intervention)

        self.interventions = lstInterventions
